...
export function DateCell({ date, remaining, onSelect }: DateCellProps) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "w-full h-full rounded-xl border p-3 sm:p-4 text-left flex flex-col gap-2 transition-all duration-200 shadow-sm",
        ...
        {Array.from({ length: 5 }).map((_, idx) => {
          ...
          return (
            <div
              key={idx}
              className={cn(
                "h-2.5 flex-1 rounded-md transition-colors",
